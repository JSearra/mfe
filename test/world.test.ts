...
describe('world', () => {
  ...
  it('rejects a stale handle after the index is recycled', () => {
    ...
    const second = spawn(world, 9, 9, 0);
  ...
  it('ignores a destroy naming a stale handle', () => {
    const world = createWorld(4, 1);
    const first = spawn(world, 0, 0, 0);
    destroy(world, first);
    flushDestroys(world);

    const second = spawn(world, 0, 0, 0);
  ...
  it('flushes destroys in ascending index order regardless of destroy order', () => {
    const build = (order: number[]) => {
      ...
      const handles = Array.from({ length: 8 }, (_, i) => spawn(world, i, i, 0));
  ...
  it('allocates predictably and reports exhaustion', () => {
    ...
    expect(handleIndex(spawn(world, 0, 0, 0))).toBe(0);
    expect(handleIndex(spawn(world, 0, 0, 0))).toBe(1);
    expect(handleIndex(spawn(world, 0, 0, 0))).toBe(2);
    expect(spawn(world, 0, 0, 0)).toBe(NULL_HANDLE);
